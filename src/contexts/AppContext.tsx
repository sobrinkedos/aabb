...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
  ...
  const updateInventoryItem = async (updatedItem: InventoryItem & { isCorrection?: boolean; correctionNotes?: string }) => {
    ...
    // Buscar dados anteriores para registrar movimentação
    ...
    // CORREÇÃO CRÍTICA: Para correções, o estoque já foi atualizado no itemToUpdate
    // Não devemos usar register_inventory_movement que vai aplicar a diferença novamente!
    ...
    if (data) {
      ...
      // Registrar movimentação se houve mudança no estoque ou custo
      ...
      if (stockChanged || costChanged) {
        ...
        if (isCorrection) {
          // Para correções, apenas registrar movimentação informativa (estoque já foi atualizado)
          console.log('🔧 Processando como CORREÇÃO - apenas registrando histórico');
          
          const notaCorrecao = updatedItem.correctionNotes || 'Correção de valores via edição de item';
          
          if (stockChanged) {
            console.log('📝 Registrando movimentação de correção no histórico');
            
            // Função auxiliar para registrar movimentação de correção
            const registrarMovimentacaoCorrecao = async () => {
              try {
                console.log('📝 Dados da movimentação de correção:', {
                  empresa_id: empresaId,
                  ...
                  notes: `CORREÇÃO - ${notaCorrecao} | Estoque: ${estoqueAnterior} → ${estoqueNovo}`,
                  created_by: user.id
                });
                
                // Tentar inserção direta (usando 0.001 para correções devido à constraint positive_quantity)
                const { data: movementData, error: movementError } = await supabase
                  ...
                  .insert({
                    ...
                    inventory_item_id: updatedItem.id,
                    movement_type: 'entrada_ajuste',
                    quantity: 0.001, // Quantidade mínima para correções (constraint positive_quantity)
                    unit_cost: custoNovo || null,
                    stock_before: estoqueAnterior,
                    stock_after: estoqueNovo,
                    notes: `CORREÇÃO - ${notaCorrecao} | Estoque: ${estoqueAnterior} → ${estoqueNovo}`,
                ...
                if (movementError) {
                  console.error('❌ Erro na inserção direta:', movementError);
                  
                  // Tentar com quantidade 1 em vez de 0 (pode ser uma validação)
                  console.log('🔄 Tentando com quantity = 1...');
                  const { data: movementData2, error: movementError2 } = await supabase
                    ...
                    .insert({
                      ...
                      stock_before: estoqueAnterior,
                      stock_after: estoqueNovo,
                      notes: `CORREÇÃO - ${notaCorrecao} | Estoque: ${estoqueAnterior} → ${estoqueNovo} (registro de auditoria)`,
                      reference_document: null,
                      created_by: user.id
                    })
                    .select();
                  ...
                  if (movementError2) {
                    console.error('❌ Erro na segunda tentativa:', movementError2);
                    alert('Aviso: O estoque foi atualizado, mas houve problema ao registrar no histórico.');
                  } else {
                    console.log('✅ Movimentação registrada na segunda tentativa!', movementData2);
                  }
                } else {
                  console.log('✅ Movimentação de correção registrada com sucesso!', movementData);
                }
              } catch (error) {
                console.error('❌ Erro geral ao registrar movimentação:', error);
                alert('Aviso: O estoque foi atualizado, mas houve problema ao registrar no histórico.');
              }
            };
          ...
          if (costChanged) {
            notaDetalhada += ` | Custo alterado: R$ ${Number(custoAnterior || 0).toFixed(2)} → R$ ${Number(custoNovo || 0).toFixed(2)}`;
          }
          ...
          try {
            const { error: movementError } = await supabase.rpc('register_inventory_movement', {
              ...
              p_quantity: 0,
              p_unit_cost: custoNovo || null,
              p_notes: `${notaCorrecao} | Custo: R$ ${Number(custoAnterior || 0).toFixed(2)} → R$ ${Number(custoNovo || 0).toFixed(2)}`,
              p_reference_document: null,
              p_created_by: user.id
            });
